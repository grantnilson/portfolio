...
function WorkExperience({ experiences }: Props) {
  return (
    <div>
      <h3 className="absolute invisible md:visible text-center top-24 uppercase tracking-[20px] text-gray-500 text-2xl shrink">
        Experience
      </h3>
      <motion.div
        initial={{ opacity: 0 }}
      ...
      >
        <div className="w-full top-24 flex space-x-5 overflow-x-scroll p-10 snap-x snap-mandatory scrollbar scrollbar-track-gray-400/20 scrollbar-thumb-[#F7AB0A]/80">
          {experiences?.map((experience) => (
            <FirstExperienceCard key={experience._id} experience={experience} />
          ))}
        </div>
      </motion.div>
    </div>
